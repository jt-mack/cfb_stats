# ESPN / SportsDataverse Contract Diagnostic Suite

On-demand checks that compare live SportsDataverse / ESPN responses against repo-owned mapping, repos, and the domain fields the frontend reads.
...
Uses `tsx --test` against [`lib/espn-contract/suite.test.ts`](../lib/espn-contract/suite.test.ts).
...
2. Owner-repo mapper output
3. Repo / domain output ([`lib/types.ts`](../lib/types.ts))
4. UI field inventory ([`lib/espn-contract/ui-fields.ts`](../lib/espn-contract/ui-fields.ts))
...
1. Run the suite
2. Triage failures: types → owning repo maps → repos → frontend types/usage
3. Re-run until unexplained failures for the **prior** completed season are cleared

## Known infrastructure notes

- ESPN `site.api` returns **403** when sportsdataverse’s default User-Agent is sent. [`lib/espn-client.ts`](../lib/espn-client.ts) patches `axios.create` to omit User-Agent on ESPN hosts before loading SDV.
- Historical rankings no longer use legacy `getRankings` (HTML). Prior seasons use Core `espnCfbSeasonWeekRankings` + ranking detail refs (see [`repos/rankings-repo.ts`](../repos/rankings-repo.ts)).

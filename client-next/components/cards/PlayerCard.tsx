...
export function PlayerCard({ player, stats }: PlayerCardProps) {
  const displayName = player.fullName ?? player.displayName ?? player.name ?? "—";
  const imgSrc =
    ...
    <Card className="overflow-hidden gap-0 py-0">
      {player.team?.logo && (
        <CardHeader className="py-1 px-2 flex flex-row items-center justify-between">
          ...
          <img src={player.team.logo} alt="" className="h-6 w-auto object-contain" />
      ...
      <CardContent className="p-2 text-center">
        ...
        <p className="text-xs font-semibold text-muted-foreground mt-1">{statType}</p>
        <p className="text-xs text-muted-foreground">{String(statValue)}</p>
      </CardContent>
    </Card>
  );
}
